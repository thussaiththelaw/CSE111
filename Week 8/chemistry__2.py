#Zak Law
#this program takes inputs from a user of a chemical formula and calculates molar mass and how many moles that
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for i in symbol_quantity_list:
        #gets the molecular name
        molecular_name = i[NAME_INDEX]
        #gets quantity from the current itteration of the different molecules
        quantity = i[1]
        #uses the molecular name to get infromation from periodic table dictionary
        element_attributes = periodic_table_dict[molecular_name]
        #takes atomic mass from the gathered information of the element
        atomic_weight = element_attributes[ATOMIC_MASS_INDEX]
        #calculate the total molar mass of the chemical compound
...
def is_it_known(chemical_formula, known_molecules_dict):
    #if the chemical formula is in the known molecules dictionary
    if chemical_formula in known_molecules_dict:
        #the molecular name is the information stored under the chemical compound's formula
        molecule_name = known_molecules_dict[chemical_formula]
    #but if it isn't in the known molecules list
    else:
        #set the molecule name to unknown molecule
        molecule_name = 'unkown molecule'
    return molecule_name

def main():
    # Get a chemical formula for a molecule from the user.
    chemical_formula = input("What is the chemical formula? ex:'H2O', 'C6H12O6': " )
    # Get a mass in grams from the user.
    mass_of_chemical_input = float(input("What is the mass of your sample?: "))
    ...
    moles_of_sample = mass_of_chemical_input/molar_mass
    # Print the molar mass.
    print (molar_mass)
    # Print the number of moles.
    print (moles_of_sample)
    known_compound = is_it_known(chemical_formula, known_molecules_dict)
    print (known_compound)
