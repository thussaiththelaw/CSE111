#Zak Law
#This program gives a 10% discount if the subtotal is greater than $50 and it is Tuesday or Wednesday, then it adds a 6% sales tax to the ending total
...
def discount_on_subtotal(): #function that multiplies the total by .9 to get 10% off the total price
    ...
    global discount
    discount = subtotal * .1
    sales_tax_on_subtotal() #move on to calculate sales tax on discounted total

def sales_tax_on_subtotal(): #function that calculates the sales tax on the total
    global subtotal
    global sales_tax
    sales_tax = (subtotal * .06)
    total_sale() #move onto final calculation

def total_sale(): #final calculation of total
    global subtotal
    global discount
    global sales_tax
    global total
...
if discount_day == True: #if it is Tuesday or Wednesday
    if subtotal >= 50: #and the subototal is equal to or greater than $50
        ...
        sales_tax_on_subtotal() # go through the functions without the discount
